...
- `model`/`model_reasoning_effort`：**梁师傅要求单独控制、不跟随真实账号**——非 owner 走这个高频路径，用真实账号那个 `high` 推理强度档位成本会明显更高。两项都已实测确认可以按 thread 独立覆盖（详见「测试计划」一节的实测记录），`model` 走 weclaw 已有的 `agents.codex-shared.model` 字段（`ACPAgentConfig.Model` 早就有了，不用新增字段），`model_reasoning_effort` 是新增字段（见下方「存储」一节），具体档位留给梁师傅在 config.json 里配置决定，本设计不预设具体模型名/档位。
...
`config.AgentConfig` 新增一个字段，跟已有的 `Model string` 并列：
```go
ModelReasoningEffort string `json:"model_reasoning_effort,omitempty"` // "none"|"low"|"medium"|"high"|"xhigh"，传给 codex thread/start 的 config 覆盖对象
```
...
- **已实测确认（写计划前的前置任务已完成）**：`model` 和 `model_reasoning_effort` 都能通过 `thread/start` 独立生效——`model` 是顶层参数（`{"model":"gpt-5.6-terra"}`，回复正确用了指定模型）；`model_reasoning_effort` 走 `config` 覆盖对象（`{"config":{"model_reasoning_effort":"low"}}`，`thread/start` 响应里 `reasoningEffort` 字段正确回显，回复正常）——注意合法值是 `none`/`low`/`medium`/`high`/`xhigh`，`minimal` 不是合法值（真测的时候传错过一次，报错信息里带出了合法值列表）。这条证明"`config` 覆盖对象"这个机制本身是通的，只是 `project_doc_max_bytes` 这个具体 key 对全局 AGENTS.md 加载不起作用（两码事，不能因为一个 key 不管用就推断整个覆盖机制不管用，也不能反过来假设都管用——都是一个个测出来的）。
- `config.AgentConfig` 新增 `ModelReasoningEffort string`（`json:"model_reasoning_effort,omitempty"`），跟现有 `Model string` 字段并列，用法一致：`codex-shared` 条目里配了就传给 `thread/start`，不配就用 codex 默认。**后台 18022 页面暂不为这个（或现有的 `Model`）做图形化开关**——现状是所有 agent 的模型档位都靠手改 `config.json`，这次保持一致，不新增专门的 UI，后续梁师傅要的话再单独提。
